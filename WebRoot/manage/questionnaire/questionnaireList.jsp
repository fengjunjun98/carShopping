	...
	<s:form action="questionnaire" namespace="/manage" method="post"
		...
		<table class="table table-bordered">
			<tr>
				...
				<td style="text-align: left;"><s:textfield name="e.title" cssClass="search-query input-small"
						id="title" /></td>
				<td style="text-align: right;">状态</td>
				<td style="text-align: left;">
					<s:select list="#{'':'','y':'已启用','n':'已禁用'}" id="status" name="e.status"  cssClass="input-medium" 
			...
			<tr>
				<td colspan="16">
					<s:submit method="selectList" value="查询" cssClass="btn btn-primary"></s:submit>
					<s:submit method="toAdd" action="questionnaire" value="添加" cssClass="btn btn-success" />
...
<%-- 							<s:submit method="updateInBlackList" onclick="return updateInBlackList();" value="加入黑名单" cssClass="btn btn-warning" /> --%>
					<s:submit method="setStatusY" value="显示" onclick="return deleteSelect('确定让选择的记录审核通过?这样选择的记录将会出现在门户上。');" cssClass="btn btn-warning" />
					<s:submit method="setStatusN" value="不显示" cssClass="btn btn-warning" onclick="return deleteSelect('执行该操作后,选择的记录将不会出现在门户上。确定要执行?');"/>
