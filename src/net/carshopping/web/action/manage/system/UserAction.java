...
/** 项目名称：carShopping 
 * 类名称：UserAction 后台用户管理
 * 类描述：b2b,b2c，处理页面提交的请求。 
 * 类描述：Bean,响应Action请求，对数据库进行操作，并把结果返回到Action。 
 * Copyright: Copyright (c) 2015 by 芜湖领悟信息科技有限公司
 * Company: b2c b2b game trading platform System 
 * 创建人：凤军军
 * 创建时间：2015-2-6 下午03:14:23 
 * 修改人：凤军军 
 * 修改时间：2015-2-6 下午03:14:23 
 * 修改备注：
 * @version 1.0* */
public class UserAction extends BaseAction<User> implements ModelDriven<User> {
	...
	public String loginOut() throws Exception {
		...
		getSession().setAttribute(ManageContainer.manage_session_user_info,
				null);
		getSession().setAttribute(ManageContainer.resource_menus, null);
		getSession().setAttribute(ManageContainer.user_resource_menus_button,
				null);
		e.clear();
		return Action.INPUT;
	}
